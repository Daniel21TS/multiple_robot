    ...
    def compute_density(self, frontier_robot, map_type):
        ...
        for data in frontier_robot:
            ...
            for data_1 in frontier_robot:
                ...
                if abs(middle_point.point.x - middle_point_1.point.x) < 4 and abs(middle_point.point.y - middle_point_1.point.y) < 4:
                    frontier_density += 1

            if map_type == 1:
                if size <= 60 and frontier_density <= 4:
                    size = 0.0017 
                    table.append((size, middle_point))
                if size > 60 and size <= 120 and frontier_density <= 4:
                    size = 0.0039
                    table.append((size, middle_point))
                if size > 120 and frontier_density <= 4:
                    size = 0.0035
                    table.append((size, middle_point))
                if size <= 60 and frontier_density > 4 and frontier_density <= 6:
                    size = 0.0017 
                    table.append((size, middle_point))
                if size > 60 and size <= 120 and frontier_density > 4 and frontier_density <= 6:
                    size = 0.0046
                    table.append((size, middle_point))
                if size > 120 and frontier_density > 4 and frontier_density <= 6:
                    size = 0.0060
                    table.append((size, middle_point))
                if size <= 60 and frontier_density > 6:
                    size = 0.0021
                    table.append((size, middle_point))
                if size > 60 and size <= 120 and frontier_density > 6:
                    size = 0.0042
                    table.append((size, middle_point))
                if size > 120 and frontier_density > 6:
                    size = 0.0094
                    table.append((size, middle_point))
        return table
